...
import DataUnmask from 'components/Topics/Topic/DataUnmask/DataUnmask';
import { screen, waitFor } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { render, WithRoute } from 'lib/testHelpers';
import { clusterTopicPath } from 'lib/paths';
import { validateBySchema } from 'components/Topics/Topic/DataUnmask/utils';
import { externalTopicPayload } from 'lib/fixtures/topics';
import { useDataUnmask, useTopicDetails } from 'lib/hooks/api/topics';
...
jest.mock('components/Topics/Topic/DataUnmask/utils', () => ({
  ...jest.requireActual('components/Topics/Topic/DataUnmask/utils'),
...
const renderComponent = async () => {
  ...
  await render(
    <WithRoute path={clusterTopicPath()}>
      <DataUnmask closeSidebar={mockOnSubmit} />
    </WithRoute>,
    { initialEntries: [path] }
  );
};
...
describe('DataUnmask', () => {
  ...
  describe('when schema is fetched', () => {
    it('calls sendTopicMessage on submit', async () => {
      ...
      (useDataUnmask as jest.Mock).mockImplementation(() => ({
