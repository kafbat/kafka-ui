...
class ReassignmentOperationsTest {
  ...
  ReassignmentOperations ops = new ReassignmentOperations(
      ReactiveAdminClient.create(ADMIN_CLIENT, new ClustersProperties.AdminClient()).block());
  ...
  void testValidateAndExecute() throws Exception {
    ...
    Map<TopicPartition, List<Integer>> desiredAssignment = currentAssignment.entrySet().stream()
        .map(e -> Tuples.of(e.getKey(), Lists.reverse(e.getValue())))
        .collect(toMap(Tuple2::getT1, Tuple2::getT2));

    ops.validateAndExecute(
        desiredAssignment.entrySet().stream().map(e -> Tuples.of(e.getKey(), e.getValue())).toList(), () -> {
        }).block();
