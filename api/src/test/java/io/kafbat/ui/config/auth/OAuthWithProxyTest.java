  ...
  class JwtDecoder {
    ...
    void jwksRequestGoesThruProxy() throws Exception {
      ...
      StepVerifier.create(jwtDecoder.decode(jwt.serialize()))
          .assertNext(decoded -> assertThat(decoded.getSubject()).isEqualTo("user"))
          .verifyComplete();
    ...
    void setup() {
      ...
      OAuthTestSupport.getOAuthServer().stubFor(post(urlPathEqualTo(INTROSPECT_PATH))
          .willReturn(aResponse()
              .withStatus(200)
              .withHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)
              .withBody("{\"active\":true,\"sub\":\"user\",\"username\":\"testuser\"}")));
    }

    @Test
    void introspectionRequestGoesThruProxy() {
      StepVerifier.create(introspector.introspect("opaque-token"))
          .assertNext(principal -> {
            assertThat(principal.getName()).isEqualTo("user");
            assertThat(principal.getAttributes()).containsEntry("username", "testuser");
          })
          .verifyComplete();
