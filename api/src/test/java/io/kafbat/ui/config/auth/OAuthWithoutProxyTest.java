...
class OAuthWithoutProxyTest {
  ...
  @Nested
  @SpringBootTest(
      classes = {OAuthSecurityConfig.class, OAuthTestSupport.JwtTestConfig.class},
      properties = {"spring.main.allow-bean-definition-overriding=true", "auth.type=OAUTH2"})
  @ContextConfiguration(initializers = OAuthTestSupport.WithoutProxyInitializer.class)
  @DirtiesContext
  @ActiveProfiles("test")
    ...
    void setup() {
      ...
      OAuthTestSupport.getOAuthServer().stubFor(get(urlPathEqualTo(JWKS_PATH))
          .willReturn(aResponse()
              .withStatus(200)
              .withHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)
              .withBody(new JWKSet(OAuthTestSupport.getRsaKey().toPublicJWK()).toString())));
    }

    @Test
    void jwksRequestBypassesProxy() throws Exception {
      ...
      var jwt = new SignedJWT(
          new JWSHeader.Builder(JWSAlgorithm.RS256)
              .keyID(OAuthTestSupport.getRsaKey().getKeyID()).build(),
          claims);
      jwt.sign(new RSASSASigner(OAuthTestSupport.getRsaKey()));

      StepVerifier.create(jwtDecoder.decode(jwt.serialize()))
          .assertNext(decoded -> assertThat(decoded.getSubject()).isEqualTo("direct-user"))
          .verifyComplete();
  ...
  @Nested
  @SpringBootTest(
      classes = {OAuthSecurityConfig.class, OAuthTestSupport.OpaqueTokenTestConfig.class},
      properties = {"spring.main.allow-bean-definition-overriding=true", "auth.type=OAUTH2"})
  @ContextConfiguration(initializers = OAuthTestSupport.WithoutProxyInitializer.class)
  @DirtiesContext
  @ActiveProfiles("test")
  class OpaqueToken {
    ...
    void setup() {
      OAuthTestSupport.resetServers();
      OAuthTestSupport.getOAuthServer().stubFor(post(urlPathEqualTo(INTROSPECT_PATH))
          .willReturn(aResponse()
              .withStatus(200)
              .withHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)
              .withBody("{\"active\":true,\"sub\":\"direct-user\",\"username\":\"directuser\"}")));
    }

    @Test
    void introspectionRequestBypassesProxy() {
      StepVerifier.create(introspector.introspect("opaque-token"))
          .assertNext(principal -> {
            assertThat(principal.getName()).isEqualTo("direct-user");
            assertThat(principal.getAttributes()).containsEntry("username", "directuser");
          })
          .verifyComplete();

      OAuthTestSupport.getOAuthServer().verify(
          postRequestedFor(urlPathEqualTo(INTROSPECT_PATH)).withRequestBody(containing("token=opaque-token")));
      assertThat(OAuthTestSupport.getProxyServer().getAllServeEvents()).isEmpty();
    }
  }
}
